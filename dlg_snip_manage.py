...
class DlgSnipMan:
    ...
    def _on_group_selected(self, id_dlg, id_ctl, data='', info=''):
        ...
        snip_items = '\t'.join(snip_items).splitlines()
    ...
    def _dlg_del_pkg(self, *args, **vargs):
        ...
        if not pkg:
            return
        res = ct.dlg_input(_('As a precaution Snippet Manager does not delete user files.\n'
                'To delete package "{}" - delete the following directory:').format(pkg['name']), pkg['path'])
    ...
    def _dlg_del_group(self, *args, **vargs):
        ...
        if pkg  and snips_fn:
            fstr = _('As a precaution Snippet Manager does not delete user files.\n'
